...
前端编写简单页面步骤：编写静态页面（input包括name和email）——添加事件监听（给他们添加id然后通过获取id添加input事件）——绑定事件（通过输入的值进行更新）——更新页面内容；

前端编写简单页面步骤2：新增一个卡片——对输入框进行字数限制——获取输入框内容；

前端编写逻辑：获取dom元素、通过监听dom元素去绑定事件或者更新dom元素的一些内容来去实现我们的功能；

前端框架：模板引擎/渲染引擎，前端开发代码——解析——AST对象——编译——更新页面。最大特点虚拟DOM树和避免XSS漏洞；

虚拟DOM：用JS对象模拟DOM树，得到一颗虚拟DOM树。当页面数据变更时生产新的虚拟DOM数，比较新旧两颗虚拟DOM树的差异。把差异应用到真正的DOM树上；

编译后的结果怎么更新到页面中：绑定映射表方式 / 直接替换内容的方式；

前后端联调：
![Alt text](image-37.png)![Alt text](image-38.png)![Alt text](image-39.png)![Alt text](image-40.png)
